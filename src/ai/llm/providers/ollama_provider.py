...
class OllamaProvider(LocalOllamaTransport, LLMProvider):
    """
    ...
    # Explicit operational budgets, not confidence/business thresholds. 8K fits
    # the observed 4,426-token full retry plus its 1,337-token response. Requests
    # that exceed a configured budget must fail, never discard document input.
    ...
    def __init__(self) -> None:
        ...
        self.context_tokens = self._positive_token_setting(
            "OLLAMA_CONTEXT_TOKENS", self.DEFAULT_CONTEXT_TOKENS)
        self.max_output_tokens = self._positive_token_setting(
            "OLLAMA_MAX_OUTPUT_TOKENS", self.DEFAULT_MAX_OUTPUT_TOKENS)
